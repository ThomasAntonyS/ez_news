...
const NewSection = () => {
  ...
  async function fetchPopular() {
    try {
      ...
      sessionStorage.setItem('popular', JSON.stringify(data.articles || []));
    } catch (err) {
        ...
        <div className='w-full flex flex-col lg:flex-row gap-4 '>
          ...
          <div className='w-full lg:w-[60%] rounded-md'>
            {popularNews.map((item, index) => (
              <Link key={index} className='flex flex-col sm:flex-row w-full sm:p-2 mb-16 shadow-md sm:shadow-none sm:mb-6 gap-3 transition-all rounded-lg sm:rounded-none text-justify hover:border-b'
              to={item.url} target='_blank' rel="noopener noreferrer"
              >
                <img
                  src={item.image}
                  alt={item.title}
          ...
          <div className='w-full lg:w-[40%] h-max rounded-md p-3'>
            ...
            {trending.map((item, index) => (
              <Link key={index} className='flex items-start gap-3 py-4'
               to={item.url} target="_blank" rel="noopener noreferrer">
                <div className='text-white bg-red-600 px-2 py-1 rounded-sm text-sm'>{index + 1}</div>
                <div className='flex flex-col justify-between w-full '>
                  <p className='font-semibold text-base sm:text-lg line-clamp-2 hover:text-blue-400 hover:cursor-pointer text-justify'>{item.title}</p>
